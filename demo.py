...
def create_animation(source_image, mode):
    ...
    if(mode == '1'):
        driving_video_mode = '.\\data\\vid\\eye.mp4' # Nhắm mắt
    if(mode == '2'):
        driving_video_mode = '.\\data\\vid\\smile.mp4' # Cười
    if(mode == '3'):
        driving_video_mode = '.\\data\\vid\\around.mp4' # Xoay trái
    if(mode == '4'):
        driving_video_mode = '.\\data\\vid\\around.mp4' # Xoay phải
    ...
    if find_best_frame_mode or best_frame is not None:
        ...
        predictions = make_animation(source_image, driving_video, generator, kp_detector, relative=True, adapt_movement_scale=True, cpu=cpu)
    imageio.mimsave(video_name, [img_as_ubyte(frame) for frame in predictions], fps=fps)
